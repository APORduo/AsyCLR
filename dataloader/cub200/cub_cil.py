...
class CubCil(CUB200):
    def __init__(self, root: str = '/data',
        ...
        super().__init__(root ,train=train,base_sess=True,index=index)
        
        
        base_classes = range(100)
        base_index = [i for i in range(len(self.targets)) if self.targets[i] in base_classes]
        ...
        if train:
            ...
            prefix = os.path.join(self.root, 'CUB_200_2011/images')
            #paths = [i[0] for i in self.imgs] #images 包含路径和标签
            for k,v in FSCIL_SAMPLES.items():
                for i in range(len(v)):
                    v[i] = os.path.join(prefix,k, v[i])
                    #breakpoint()
                    index = self.data.index((v[i]))
                    train_index.append(index)
            self.data = np.array(self.data)[train_index]
            self.targets = np.array(self.targets)[train_index]
            #self.imgs = self.imgs[train_index]
         
            
        
if __name__ == '__main__':
    cub = CubCil()
    cub_test = CubCil(train=False)
    
    data = cub_test.data
    target = cub_test.targets
    #breakpoint()
    name = [data.split('/')[4] for data in data]
    name = sorted(set(name))
    fo = open('cubcil_test.json','w'  )
    fo.write('{"label_names": [')
    fo.writelines(['"%s",' % item  for item in name])
    ...
    fo.writelines(['%d,' % item  for item in target])
    fo.seek(0, os.SEEK_END)
    fo.seek(fo.tell()-1, os.SEEK_SET)
    fo.write(']}')
